...
df['Prob_String'] = [", ".join([f"G{i}: {p:.1%}" for i, p in enumerate(row)]) for row in probs]

# --- Metrics ---
m1, m2, m3 = st.columns(3)
with m1:
    st.markdown(f"""<div class="metric-card"><div class="metric-label">Likelihood (Log)</div><div class="metric-value">{gmm.lower_bound_:.2f}</div></div>""", unsafe_allow_html=True)
with m2:
    st.markdown(f"""<div class="metric-card"><div class="metric-label">Avg Confidence</div><div class="metric-value">{df['Confidence'].mean():.1%}</div></div>""", unsafe_allow_html=True)
with m3:
    st.markdown(f"""<div class="metric-card"><div class="metric-label">Total Players</div><div class="metric-value">{len(df)}</div></div>""", unsafe_allow_html=True)
...
with col_plot:
    ...
    scatter = alt.Chart(df).mark_circle(size=100, stroke='white', strokeWidth=0.5).encode(
        x=alt.X('Rush:Q', title="Rushing Yards"),
        y=alt.Y('Rec:Q', title="Receiving Yards"),
        color=alt.Color('Gaussian_Label:N', scale=color_scale, title="Most Likely Gaussian"),
        size=alt.Size('Confidence:Q', scale=alt.Scale(range=[40, 300]), title="Confidence Score"),
        opacity=alt.condition(alt.datum.Confidence < 0.6, alt.value(0.5), alt.value(0.9)),
        tooltip=['True Role', 'Rush', 'Rec', 'Gaussian_Label', 'Prob_String']
    ).properties(width='container', height=600).interactive()

    st.altair_chart(scatter, width='stretch')

with col_analysis:
    st.subheader("Probabilistic Analysis")
    st.write("GMM is perfect for **Hybrid Roles**. Look for players with smaller dots—these are players the model is unsure about.")
    
    # Show lowest confidence players (the hybrids)
    low_conf = df.sort_values('Confidence').head(5)[['True Role', 'Confidence', 'Gaussian_Label']]
    ...
    if covariance_type == "full":
        st.write("Using **Full** covariance allows the 'clouds' to rotate. This captures the correlation between rushing and receiving.")
    elif covariance_type == "diag":
        st.write("Using **Diag** covariance forces the 'clouds' to align with the axes. This ignores correlation.")
...
st.markdown("---")
st.caption("Gaussian Mixture Models use the Expectation-Maximization (EM) algorithm to find the parameters of the distributions that best represent the data.")