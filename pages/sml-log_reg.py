...
with st.sidebar:
    ...
    mode = st.radio("Model Logic", ["Exact (Solver)", "Iterative (Gradient Descent)"], 
                    help="Exact finds the best lines instantly. Iterative shows the model learning step-by-step.")

    st.markdown("---")
    st.subheader("2. Model Tuning")
    
    c_param = st.slider("Inverse Regularization (C)", 0.01, 10.0, 1.0, help="Lower C = Stronger Regularization (smoother lines).")
    
    use_balanced = st.checkbox("Balanced Class Weights", value=True, 
    ...
    current_config = {"roles": sorted(selected_roles), "poly": use_poly, "C": c_param, "seed": seed_val, "balanced": use_balanced}
    if "last_config" not in st.session_state or st.session_state.last_config != current_config:
        ...
        st.session_state.lr_history = []
        st.session_state.lr_converged = False
        if 'lr_model' in st.session_state: del st.session_state.lr_model
...
feature_names = poly.get_feature_names_out(['Rush', 'Rec']) if use_poly else ['Rush', 'Rec']

# --- Model Logic ---
class_weight = 'balanced' if use_balanced else None

if mode == "Exact (Solver)":
    model = LogisticRegression(C=c_param, solver='lbfgs', max_iter=1000, class_weight=class_weight)
    ...
    if 'lr_model' not in st.session_state:
        ...
        model.coef_ = np.random.randn(len(classes) if len(classes) > 2 else 1, X_input.shape[1]) * 2.0
        model.intercept_ = np.random.randn(len(classes) if len(classes) > 2 else 1) * 2.0
...
if mode == "Iterative (Gradient Descent)":
    if st.session_state.lr_current_iteration >= max_iter:
        ...
        status_text = "🟡 Idle"
    st.markdown(f"**Iteration:** `{st.session_state.lr_current_iteration} / {max_iter}` | Status: {status_text}")
else:
    st.markdown(f"**Status:** ✅ Optimal Boundary Found (Exact Mode)")
...
grid_df = pd.DataFrame({'Rush': xx.ravel(), 'Rec': yy.ravel(), 'Rush2': xx.ravel()+h, 'Rec2': yy.ravel()+h, 'Predicted_Role': Z})
        ...
        for j in range(i + 1, len(classes)):
            ...
            if abs(dw[1]) > 1e-5:
                ...
                for s in range(samples - 1):
                    ...
                    # Wrap in DataFrame to keep feature names and avoid UserWarning
                    mid_scaled = scaler.transform(pd.DataFrame([[mid_x, mid_y]], columns=['Rush', 'Rec']))
...
with col_viz:
    ...
    with m1: st.markdown(f'<div class="metric-card"><div class="metric-label">Accuracy</div><div class="metric-value">{accuracy:.1%}</div></div>', unsafe_allow_html=True)
    with m2: st.markdown(f'<div class="metric-card"><div class="metric-label">Weights</div><div class="metric-value">{model.coef_.size}</div></div>', unsafe_allow_html=True)
    with m3: st.markdown(f'<div class="metric-card"><div class="metric-label">Training Size</div><div class="metric-value">{len(df)}</div></div>', unsafe_allow_html=True)

    color_scale = alt.Scale(domain=selected_roles, scheme='tableau10')
    background = alt.Chart(grid_df).mark_rect(opacity=0.3).encode(
        x=alt.X('Rush:Q', title="Rushing Yards", scale=alt.Scale(domain=[0, x_max])), x2='Rush2:Q',
        y=alt.Y('Rec:Q', title="Receiving Yards", scale=alt.Scale(domain=[0, y_max])), y2='Rec2:Q',
    ...
    points = alt.Chart(df).mark_circle(size=75, stroke='black', strokeWidth=0.5).encode(
        x='Rush:Q', y='Rec:Q', color=alt.Color('True Role:N', scale=color_scale, title="Real Role"),
    ...
    if not lines_df.empty:
        ...
        layers.append(alt.Chart(lines_df[lines_df['Active'] == 'Solid']).mark_rule(
            color='white', opacity=0.8, strokeWidth=2
        ).encode(x='x1:Q', y='y1:Q', x2='x2:Q', y2='y2:Q'))
        
        # Plot inactive lines (dotted, thin)
        layers.append(alt.Chart(lines_df[lines_df['Active'] == 'Dotted']).mark_rule(
            color='white', opacity=0.3, strokeWidth=1, strokeDash=[4, 4]
        ).encode(x='x1:Q', y='y1:Q', x2='x2:Q', y2='y2:Q'))

    st.altair_chart(alt.layer(*layers).properties(width='container', height=500).interactive(), width='stretch')
...
with col_rules:
    ...
    st.markdown(f'<div class="coef-container">{coef_text}</div>', unsafe_allow_html=True)
    
# --- History Plot ---
if mode == "Iterative (Gradient Descent)" and st.session_state.lr_history:
    st.markdown("---")
    st.subheader("📈 Accuracy Over Time")
    hist_df = pd.DataFrame({'Iteration': range(len(st.session_state.lr_history)), 'Accuracy': st.session_state.lr_history})
    st.altair_chart(alt.Chart(hist_df).mark_line(color='#f87171').encode(x='Iteration', y='Accuracy').properties(width='container', height=150), width='stretch')
...
if mode == "Iterative (Gradient Descent)" and st.session_state.lr_is_training:
    if st.session_state.lr_current_iteration < max_iter:
        ...
        sample_weights = compute_sample_weight('balanced', y_encoded) if use_balanced else None
        st.session_state.lr_model.partial_fit(X_input, y_encoded, classes=classes, sample_weight=sample_weights)
        st.session_state.lr_current_iteration += 1
        st.session_state.lr_history.append(st.session_state.lr_model.score(X_input, y_encoded))
        st.rerun()
    else:
        st.session_state.lr_is_training = False
        st.rerun()