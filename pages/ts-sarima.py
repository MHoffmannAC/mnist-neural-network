...
with st.sidebar:
    ...
    validation_mode = st.toggle("Validation Mode (Backtest)", value=False, 
    ...
    if validation_mode:
        st.info(f"Training on data up to {val_window} {resample_freq} steps ago. Testing on final year.")
        periods = val_window
    else:
        horizon_max = 365 if current_freq == "D" else (104 if current_freq == "W" else 24)
        if current_freq == "YE":
            horizon_max = 5
        periods = st.slider(f"{resample_freq} Steps to Forecast", 1, int(2*horizon_max), int(horizon_max))
    ...
    d = st.slider("d (Degree of Differencing)", 0, 3, 0, 
    ...
    if diag_decomp:
        decomp_mode = st.radio("Decomp Mode", ["additive", "multiplicative"], index=0, horizontal=True, label_visibility="collapsed")
    ...
    if diag_adf:
        adf_diff_order = st.slider("Differencing Order for ADF Test", 0, 3, 0, 
                                   help="Check if first or second order differencing makes the series stationary.")

    st.markdown("---")
    st.subheader("5. Visualization View")
    view_mode = st.radio("Display Window", ["Full History", "Last 3 Years", "Last 12 Months"])
    ...
    try:
        if m_type == "AR (AutoRegressive)":
            if d == 0:
                ...
                res = model.fit()
                forecast = res.predict(start=len(train_series), end=len(train_series) + steps - 1)
                ...
                model = SARIMAX(train_series, order=(p, d, 0), seasonal_order=(0,0,0,0))
            ...
            if s > len(train_series) / 2:
                return None, None, f"Seasonal period {s} too large for training size.", None
...
with st.spinner("Calculating statistical projections..."):
    forecast_vals, model_aic, model_summary, actual_vals = run_timeseries_engine(
        df_processed, model_type, p, d, q, P, D, Q, s, periods, validation_mode
    )

# --- Data Preparation for Visualization ---
if forecast_vals is not None:
    # 1. Create the Forecast/Prediction Dataframe
    last_train_date = df_processed['ds'].iloc[-periods-1] if validation_mode else df_processed['ds'].max()
    future_dates = pd.date_range(start=last_train_date, periods=periods + 1, freq=current_freq)[1:]
...
if validation_mode:
    ...
    with m3:
        st.markdown('<div class="metric-card"><div class="metric-label">Validation Period</div><div class="metric-value">Last Year</div></div>', unsafe_allow_html=True)
else:
    with m1:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Model AIC</div><div class="metric-value">{model_aic:.1f}</div></div>', unsafe_allow_html=True)
    with m2:
        peak_f = np.max(forecast_vals)
        st.markdown(f'<div class="metric-card"><div class="metric-label">Projected Peak</div><div class="metric-value">{peak_f:.1f}</div></div>', unsafe_allow_html=True)
    with m3:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Observation Points</div><div class="metric-value">{len(df_processed)}</div></div>', unsafe_allow_html=True)

st.markdown("<br>", unsafe_allow_html=True)

# Main Forecast Plot
st.subheader("📅 Accuracy Check: Actual vs. Predicted" if validation_mode else f"📅 {resample_freq} Interest Projection")
...
present_types = [t for t in color_map.keys() if t in plot_df['Type'].unique()]
...
base = alt.Chart(plot_df).encode(
    ...
    color=alt.Color('Type:N', scale=alt.Scale(domain=color_domain, range=color_range), legend=alt.Legend(title="Data Series")),
    tooltip=[alt.Tooltip('ds:T', title='Date'), alt.Tooltip('y:Q', title='Score', format='.1f'), alt.Tooltip('Type:N')]
)
...
final_chart = alt.layer(historical_line, prediction_line, ground_truth_points).properties(
...
if any([diag_decomp, diag_corr, diag_adf]):
    ...
    if diag_decomp:
        ...
        st.write(f"Isolating Trend and Seasonality using **{decomp_mode.capitalize()}** logic (Period: {yearly_period}).")
        ...
        try:
            ...
            decomp = seasonal_decompose(working_series, model=decomp_mode, period=yearly_period)
            ...
            decomp_chart = alt.Chart(decomp_df).mark_line(color='#60a5fa').encode(
                x=alt.X('ds:T', title=None),
                y=alt.Y('Value:Q', title=None),
                row=alt.Row('Component:N', sort=['Trend', 'Seasonal', 'Residual'], title=None)
            ).properties(width='container', height=150).interactive().resolve_scale(
                y='independent'
            )
    ...
    if diag_corr:
        ...
        with c1:
            ...
            acf_chart = alt.Chart(corr_df).mark_bar(size=1 if n_lags_to_show > 200 else 3, color='#60a5fa').encode(
                x='Lag:O', y='ACF:Q'
            )
            conf_band = alt.Chart(corr_df).mark_area(opacity=0.1, color='#94a3b8').encode(
                x='Lag:O', y=alt.datum(-conf_interval), y2=alt.datum(conf_interval)
            )
            st.altair_chart((conf_band + acf_chart).properties(height=250), width='stretch')
            
        with c2:
            st.markdown("**PACF (Partial Autocorrelation)**")
            pacf_chart = alt.Chart(corr_df).mark_bar(size=1 if n_lags_to_show > 200 else 3, color='#f87171').encode(
                x='Lag:O', y='PACF:Q'
            )
            st.altair_chart((conf_band + pacf_chart).properties(height=250), width='stretch')
...
with col_info:
    ...
    if validation_mode:
        ...
        **Validation Window:**
        ...
        st.write("Tune model parameters to minimize AIC and improve forecasting behavior.")

    st.info("💡 **Diagnostics Tip:** If the AR model shows a poor fit on non-stationary data, try increasing the 'd' parameter to perform an ARI model.")

with col_sum:
    st.subheader("📝 Statistical Summary")
    st.markdown(f'<div class="summary-box">{model_summary}</div>', unsafe_allow_html=True)

st.markdown("---")
st.caption("Validation mode uses the final year of data for accuracy checking. Future forecasts are hidden in this mode.")