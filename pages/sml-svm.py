    ...
    if mode == "Iterative (SGD)":
        ...
        with col2:
            if st.button("♻️ Reset"):
                ...
                st.session_state.history = []
                st.session_state.converged = False
                if "iter_model" in st.session_state:
                    del st.session_state.iter_model
                st.rerun()
    else:
...
if mode == "Exact (SVC)":
    ...
    if "iter_model" not in st.session_state:
        ...
        iter_model = SGDClassifier(
            loss="hinge",
            alpha=1 / (c_param * len(df)),
...
if mode == "Iterative (SGD)":
    status_text = (
        ...
        else ("✅ Converged" if st.session_state.converged else "🟡 Idle")
    )
    st.markdown(
        f"**Iteration:** `{st.session_state.current_iteration}` | Status: {status_text}",
    )
else:
...
with m1:
    st.markdown(
        f"""<div class="metric-card"><div class="metric-label">Current Accuracy</div><div class="metric-value">{accuracy:.1%}</div></div>""",
        unsafe_allow_html=True,
    )
with m2:
    val = (
        len(model.support_)
        if mode == "Exact (SVC)"
        else st.session_state.current_iteration
    )
    lbl = "Support Vectors" if mode == "Exact (SVC)" else "Iterations"
    st.markdown(
        f"""<div class="metric-card"><div class="metric-label">{lbl}</div><div class="metric-value">{val}</div></div>""",
        unsafe_allow_html=True,
    )
with m3:
    st.markdown(
...
st.altair_chart(
    alt.layer(*layers).properties(width="container", height=600).interactive(),
    width="stretch",
)
...
if (
    ...
    and not st.session_state.converged
):
...
with col_exp2:
    if mode == "Iterative (SGD)" and st.session_state.history:
        hist_df = pd.DataFrame(
            {
                "Iteration": range(len(st.session_state.history)),
        ...
        st.write(
            "In Exact mode, the players with white halos are the 'boundary' players. Only these players matter for the model's logic.",
        )
