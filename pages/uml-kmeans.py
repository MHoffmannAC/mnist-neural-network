...
def get_initial_centroids(df, k):
    if df.empty: return pd.DataFrame(columns=['Rush', 'Rec'])
    return df[['Rush', 'Rec']].sample(k).reset_index(drop=True)

def assign_clusters(df, centroids):
    if df.empty or centroids.empty: return np.array([])
    # Vectorized distance calculation
    dists = np.sqrt(((df[['Rush', 'Rec']].values[:, np.newaxis] - centroids.values)**2).sum(axis=2))
...
with st.sidebar:
    ...
    k = st.slider("Number of Clusters (K)", 2, 8, min(len(selected_roles), 8) if selected_roles else 4)
...
if not selected_roles:
    st.warning("Please select at least one player group in the sidebar to generate data.")
    st.stop()
...
with col_info:
    ...
    if st.button("Next Step ➡️") and not st.session_state.converged:
        if st.session_state.phase == "Assign":
            st.session_state.df['Cluster'] = assign_clusters(st.session_state.df, st.session_state.centroids)
            st.session_state.phase = "Update"
        else:
            old_centroids = st.session_state.centroids.copy()
            st.session_state.centroids = compute_new_centroids(st.session_state.df, k).combine_first(old_centroids)
            st.session_state.iteration += 1
            st.session_state.phase = "Assign"
            
            # Check for convergence
            if np.allclose(old_centroids.values, st.session_state.centroids.values, atol=1e-2):
    ...
    if st.session_state.iteration > 0:
        ...
        for i, center in st.session_state.centroids.iterrows():
            pts = st.session_state.df[st.session_state.df['Cluster'] == i][['Rush', 'Rec']]
            if not pts.empty:
...
with col_plot:
    ...
    plot_df['Cluster_Label'] = plot_df['Cluster'].apply(lambda x: f"Cluster {x}" if x >= 0 else "Unassigned")
    
    # 1. Player Points
    points = alt.Chart(plot_df).mark_circle(size=80, opacity=0.7).encode(
        x=alt.X('Rush:Q', title="Rushing Yards"),
        y=alt.Y('Rec:Q', title="Receiving Yards"),
        color=alt.Color('Cluster_Label:N', 
                       scale=alt.Scale(domain=[f"Cluster {i}" for i in range(k)] + ["Unassigned"],
                                     range=['#10b981', '#3b82f6', '#f59e0b', '#ef4444', '#a78bfa', '#ec4899', '#64748b']),
    ...
    centroid_plot_df['Cluster_Label'] = centroid_plot_df['Cluster_ID'].apply(lambda x: f"Cluster {int(x)}")
    
    centers = alt.Chart(centroid_plot_df).mark_point(
        size=500, shape='cross', strokeWidth=5, color='white', fill='black'
    ).encode(
        x='Rush:Q',
        y='Rec:Q',
    ...
    st.altair_chart((points + centers).properties(width='container', height=550).interactive(), width='stretch')
...
st.markdown("---")
st.caption("K-Means identifies spatial patterns. Try adding 'Dual-Threat QB' vs 'Pocket QB' to see if the algorithm can separate them based purely on rushing volume.")