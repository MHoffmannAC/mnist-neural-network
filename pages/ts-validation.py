...
with st.sidebar:
    ...
    # Pre-calculate data size for stability checks to prevent IndexErrors on stale slider values
    ...
    window_type = st.radio(
        "Window Logic",
        ["Expanding Window", "Fixed/Sliding Window"],
        help="Expanding: starts at zero and grows. Fixed: slides a constant-size box.",
    )

    # Robust slider bounds checking
    min_train = int(yearly_period)
    max_train = int(max(min_train + 1, n_available - 5))

    train_size = st.slider(
        "Train Window Size",
        min_value=min_train,
        max_value=max_train,
        value=int(min(yearly_period * 2, max_train)),
        help="Minimum history needed before starting the first forecast.",
    )
    ...
    horizon = st.slider(
        "Forecast Horizon (H)",
        1,
        min(3 * yearly_period, max_horizon),
    ...
    model_choice = st.selectbox(
        "Test Model", ["Seasonal Naive", "Naive", "Global Mean"],
    )
...
if train_size + horizon > n_total:
    st.error(
        f"Configuration Impossible: Train Size ({train_size}) + Horizon ({horizon}) exceeds total data points ({n_total}).",
    )
    st.stop()
...
for fold_idx, current_end in enumerate(
    range(start_idx, n_total - horizon + 1, step_size),
):
    ...
    # Safety Check for bounds - strict stop if we exceed total data
    if test_end > n_total:
        break

    # 2. Extract Data (Slicing is safe with test_end == n_total)
    y_train = df_resampled["y"].values[train_start:train_end]
    y_test = df_resampled["y"].values[test_start:test_end]

    # 3. "Predict" using selected baseline model
    ...
    mape = np.mean(np.abs((y_test - y_pred) / np.maximum(y_test, 1e-5))) * 100
    ...
    # FIX: Use iloc[index - 1] for End timestamps to avoid out-of-bounds IndexErrors
...
if not metrics:
    st.warning(
        "No folds could be generated with the current settings. Try decreasing the Train Window Size or Step Size.",
    )
    st.stop()
...
st.markdown(f"### Simulating model performance across **{len(metrics_df)} folds**")

# Top Level Stats
m1, m2, m3 = st.columns(3)
with m1:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Avg. MAE</div><div class="metric-value">{metrics_df["MAE"].mean():.2f}</div></div>',
        unsafe_allow_html=True,
    )
with m2:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Avg. RMSE</div><div class="metric-value">{metrics_df["RMSE"].mean():.2f}</div></div>',
        unsafe_allow_html=True,
    )
with m3:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Avg. MAPE</div><div class="metric-value">{metrics_df["MAPE"].mean():.1f}%</div></div>',
        unsafe_allow_html=True,
    )
...
st.write(
    "The bars show the sliding windows, while the line shows the raw interest data to highlight seasonal coverage.",
)

# Window Bars
fold_chart = (
    alt.Chart(viz_df)
    .mark_bar(height=10 if len(metrics_df) > 20 else 20, opacity=0.4)
    .encode(
        ...
        color=alt.Color(
            "Type:N",
            scale=alt.Scale(domain=["Train", "Test"], range=["#475569", "#8b5cf6"]),
            title="Window Role",
        ),
...
raw_line = (
    alt.Chart(df_resampled)
    .mark_line(color="#64748b", strokeWidth=1.5, opacity=0.8)
    .encode(x="ds:T", y=alt.Y("y:Q", title="Raw Interest Score"))
)

# Layering with independent Y-axes and FIXED height
combined_strategy = (
    alt.layer(raw_line, fold_chart)
...
metric_to_plot = st.radio(
    "Select Metric to View", ["MAE", "RMSE", "MAPE"], horizontal=True,
)
...
with st.expander("📚 Deep Dive: Time-Series Cross-Validation Concepts", expanded=False):
    ...
    with t1:
        ...
        st.info(
            "Larger horizons are harder. Error metrics usually increase significantly as you try to look further into the future.",
        )
    with t2:
        st.write("**Train Window Size**")
        st.info(
            "If this is too small, the model won't capture the yearly NFL cycle (52 weeks). Ensure this is at least 1-2 years.",
        )
    with t3:
        st.write("**Step Size**")
        st.info(
            "Controls how many simulations we run. A small step size (e.g., 1 week) provides a very smooth error plot but takes longer to compute.",
        )

st.markdown("---")
st.caption(
    "Cross-Validation Playground: Visualizing the rigor of time-series backtesting.",
)
