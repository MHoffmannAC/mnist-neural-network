...
with st.sidebar:
    ...
    eps = st.slider("Epsilon (Radius)", 0.1, 2.0, 0.5, help="Maximum distance between two samples for one to be considered as in the neighborhood of the other.")
    min_samples = st.slider("Min Samples", 2, 15, 5, help="Number of samples in a neighborhood for a point to be considered a core point.")
    
    st.info("💡 **Epsilon** controls how far the algorithm looks for neighbors. **Min Samples** controls how 'crowded' an area must be to form a cluster.")
...
with col_plot:
    # Prepare Plot Labels
    df['Cluster_Label'] = df['Cluster'].apply(lambda x: f"Cluster {x}" if x >= 0 else "Outlier (Noise)")
    
    # Custom color palette: Outliers are usually dark gray/black in DBSCAN visualizations
    color_scale = alt.Scale(
        domain=[f"Cluster {i}" for i in range(max(0, n_clusters))] + ["Outlier (Noise)"],
        range=['#10b981', '#3b82f6', '#f59e0b', '#a78bfa', '#ef4444', '#ec4899', '#06b6d4', '#475569']
    )

    chart = alt.Chart(df).mark_circle(size=100, opacity=0.8, stroke='white', strokeWidth=0.5).encode(
        x=alt.X('Rush:Q', title="Rushing Yards"),
        y=alt.Y('Rec:Q', title="Receiving Yards"),
        color=alt.Color('Cluster_Label:N', scale=color_scale, title="DBSCAN Result"),
        tooltip=['True Role', 'Rush', 'Rec', 'Cluster_Label']
    ).properties(width='container', height=550).interactive()
...
if n_clusters > 0:
    with st.expander("📊 Detailed Cluster Breakdown"):
        ...
        crosstab.columns = [f"Cluster {c}" if c != -1 else "Noise" for c in crosstab.columns]
        st.table(crosstab)