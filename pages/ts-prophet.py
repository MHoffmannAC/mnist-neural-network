...
SB_ROMANS = {
    2010: "XLIV", 2011: "XLV", 2012: "XLVI", 2013: "XLVII", 2014: "XLVIII",
    2015: "XLIX", 2016: "50", 2017: "LI", 2018: "LII", 2019: "LIII",
    2020: "LIV", 2021: "LV", 2022: "LVI", 2023: "LVII", 2024: "LVIII",
    2025: "LIX", 2026: "LX", 2027: "LXI"
}
...
def get_detailed_season_flags(dates):
    ...
    is_reg[mask_future] = np.where((months[mask_future] >= 9) | (months[mask_future] <= 2), True, False)
...
def load_nfl_trends():
    ...
    df['on_season'] = is_reg # Note: 'on_season' represents regular season for backward compatibility
...
with st.sidebar:
    ...
    periods = st.slider("Days to Forecast", 30, 5*365, 365)
    ...
    fourier_order_week = st.slider("Weekly Fourier Order", 1, 15, 5, disabled=not include_weekly)
    
    st.markdown("---")
    st.subheader("3. Season Logic (Impact Type)")
    
    use_regressor = st.checkbox("Global Level Shift", value=False, 
                                help="Adds a flat 'bonus' score to the entire regular season window.")
    
    use_conditional = st.checkbox("Conditional Weekly Patterns", value=False, 
    ...
    with col_checkbox:
        use_pre_conditional = st.checkbox("Include Preseason Weekly", value=False,
    ...
    if include_superbowl:
        lower_win = st.slider("Lower Window (Before)", 0, 14, 1)
        upper_win = st.slider("Upper Window (After)", 0, 14, 1)
    ...
    view_mode = st.radio("Display Window", ["Full History", "Last 3 Years", "Last 6 Months", "Custom Range"])
    if view_mode == "Custom Range":
        min_selectable = df['ds'].min().to_pydatetime()
        max_selectable = (df['ds'].max() + datetime.timedelta(days=periods)).to_pydatetime()
        date_range = st.date_input("Select Date Range", value=(min_selectable, max_selectable), min_value=min_selectable, max_value=max_selectable)

    st.markdown("---")
    st.subheader("7. Growth Constraints")
    growth_type = st.radio("Growth Model", ["linear", "logistic"], 
                           help="Logistic allows setting a mathematical floor (0) and a saturation cap.")
    
    cap_val = 150.0
    if growth_type == "logistic":
        cap_val = st.slider("Interest Cap (Max Potential)", float(df['y'].max()), 500.0, 200.0, 
                             help="The theoretical maximum interest score the trend can reach.")
        st.caption("✨ Floor is strictly set to 0.0 in Logistic mode.")
    else:
        st.caption("⚠️ Linear mode allows values to drop below zero if seasonal pressure is high.")

# --- Optimized Model Execution ---
@st.cache_resource
def run_prophet_engine(_df, periods, include_weekly, conditional_weekly, use_pre_cond, include_yearly, fourier_year, fourier_week, 
                       mode, use_reg, reg_prior, include_sb, l_win, u_win, h_prior, flex, r_range, s_prior, 
                       growth, cap):
    ...
    if include_weekly:
        if conditional_weekly:
            m.add_seasonality(name='weekly_on_season', period=7, fourier_order=fourier_week, prior_scale=s_prior, condition_name='on_season')
            if use_pre_cond:
                m.add_seasonality(name='weekly_pre_season', period=7, fourier_order=fourier_week, prior_scale=s_prior, condition_name='pre_season')
            
            # Off-season remains its own flatter pattern
            m.add_seasonality(name='weekly_off_season', period=7, fourier_order=3, prior_scale=s_prior, condition_name='off_season')
        else:
            m.add_seasonality(name='weekly', period=7, fourier_order=fourier_week, prior_scale=s_prior)

    if include_yearly:
        m.add_seasonality(name='yearly', period=365.25, fourier_order=fourier_year, prior_scale=s_prior)
    ...
    if use_reg:
        try:
            ...
            reg_coef = coeffs[coeffs['regressor'] == 'nfl_season_regressor']['coef'].values[0]
        except: pass
...
forecast, reg_impact = run_prophet_engine(
    df, periods, include_weekly, use_conditional, use_pre_conditional, include_yearly, fourier_order_year, fourier_order_week,
    season_mode, use_regressor, regressor_prior, include_superbowl, lower_win, upper_win, 
    holiday_prior, cp_flex, cp_range, season_prior, growth_type, cap_val
)
...
plot_df = pd.concat([historical[['ds', 'y', 'Type']], forecast_only[['ds', 'yhat', 'Type']].rename(columns={'yhat': 'y'})])
...
if view_mode == "Last 6 Months":
    ...
    plot_df = plot_df[(plot_df['ds'] >= pd.to_datetime(date_range[0])) & (plot_df['ds'] <= pd.to_datetime(date_range[1]))]
    forecast_display = forecast[(forecast['ds'] >= pd.to_datetime(date_range[0])) & (forecast['ds'] <= pd.to_datetime(date_range[1]))]
else:
...
with m1:
    st.markdown(f'<div class="metric-card"><div class="metric-label">Predicted Peak Interest</div><div class="metric-value">{forecast["yhat"].max():.1f}</div></div>', unsafe_allow_html=True)
with m2:
    growth_label = "Saturation Headroom" if growth_type == "logistic" else "Base Trend Growth"
    if growth_type == "logistic":
    ...
    st.markdown(f'<div class="metric-card"><div class="metric-label">{growth_label}</div><div class="metric-value">{growth_val}</div></div>', unsafe_allow_html=True)
with m3:
    reg_label = "Season Bonus (Shift)" if season_mode == "multiplicative" else "Season Offset"
    reg_val = f"+{reg_impact:.1%}" if season_mode == "multiplicative" else f"+{reg_impact:.1f}"
    st.markdown(f'<div class="metric-card"><div class="metric-label">{reg_label}</div><div class="metric-value">{reg_val if use_regressor else "N/A"}</div></div>', unsafe_allow_html=True)
...
band = alt.Chart(forecast_display).mark_area(opacity=0.35, color='#fbbf24').encode(
    x='ds:T', y='yhat_lower:Q', y2='yhat_upper:Q',
    tooltip=[alt.Tooltip('ds:T', title='Date'), alt.Tooltip('yhat:Q', title='Predicted', format='.1f')]
)
line = alt.Chart(plot_df).mark_line().encode(
    x=alt.X('ds:T', title="Date"),
    y=alt.Y('y:Q', title="Interest Score", scale=y_axis_scale),
    color=alt.Color('Type:N', scale=alt.Scale(domain=['Actual', 'Forecast'], range=['#94a3b8', '#fbbf24'])),
    strokeDash=alt.condition(alt.datum.Type == 'Forecast', alt.value([5, 5]), alt.value([0])),
    tooltip=[alt.Tooltip('ds:T', title='Date'), alt.Tooltip('y:Q', title='Value', format='.1f')]
)
st.altair_chart((band + line).properties(width='container', height=450).interactive(), width='stretch')
...
with col_trend:
    ...
    st.altair_chart(alt.Chart(forecast).mark_line(color='#fbbf24', size=3).encode(x='ds:T', y='trend:Q').properties(width='container', height=250).interactive(), width='stretch')

with col_season:
    st.markdown("**Yearly Pattern (Fixed Calendar Cycle)**")
    if include_yearly:
        yearly_comp = forecast[['ds', 'yearly']].iloc[:365].copy()
        st.altair_chart(alt.Chart(yearly_comp).mark_line(color='#fbbf24', strokeWidth=3).encode(x=alt.X('ds:T', axis=alt.Axis(format='%b')), y='yearly:Q').properties(width='container', height=250), width='stretch')
    else:
    ...
    if include_weekly:
        ...
        if use_conditional:
            ...
            if use_pre_conditional and 'weekly_pre_season' in forecast.columns:
                ...
                pre_summary = pre_data.groupby('Day')['weekly_pre_season'].mean().reset_index()
                pre_summary['Phase'] = 'Preseason'
                pre_summary = pre_summary.rename(columns={'weekly_pre_season': 'Adjustment'})
            ...
            off_summary = off_data.groupby('Day')['weekly_off_season'].mean().reset_index()
            off_summary['Phase'] = 'Off-Season'
            off_summary = off_summary.rename(columns={'weekly_off_season': 'Adjustment'})
            all_week_data.append(off_summary)
            
            week_summary = pd.concat(all_week_data)
            
            week_chart = alt.Chart(week_summary).mark_bar().encode(
                x=alt.X('Day:N', sort=['Mon.', 'Tue.', 'Wed.', 'Thu.', 'Fri.', 'Sat.', 'Sun.'], title="Day of Week", axis=alt.Axis(labelAngle=0)),
                y=alt.Y('Adjustment:Q', title="Multiplier / Adjustment"),
                xOffset='Phase:N',
                color=alt.Color('Phase:N', scale=alt.Scale(domain=['Regular Season', 'Preseason', 'Off-Season'], range=['#fbbf24', '#f59e0b', '#475569']))
            ...
            weekly_summary = weekly_comp.groupby('Day')['weekly'].mean().reindex(['Mon.', 'Tue.', 'Wed.', 'Thu.', 'Fri.', 'Sat.', 'Sun.']).reset_index()
            st.altair_chart(alt.Chart(weekly_summary).mark_bar(color='#fbbf24').encode(x=alt.X('Day:N', sort=None, axis=alt.Axis(labelAngle=0)), y='weekly:Q').properties(width='container', height=250), width='stretch')
    else:
...
with comp_impact:
    ...
    if use_regressor:
        reg_display_col = 'extra_regressors_multiplicative' if (season_mode == 'multiplicative' and 'extra_regressors_multiplicative' in forecast.columns) else 'nfl_season_regressor'
        if reg_display_col in forecast.columns:
            reg_comp = forecast[['ds', reg_display_col]].iloc[:365*2]
            st.altair_chart(alt.Chart(reg_comp).mark_line(color='#fbbf24').encode(x='ds:T', y=f'{reg_display_col}:Q').properties(width='container', height=250).interactive(), width='stretch')
...
if include_superbowl and 'superbowl' in forecast.columns:
    ...
    sb_title_text = f"Days relative to Super Bowl {sb_roman} ({last_sb_dt.strftime('%b %Y')})"
    st.markdown(f"**{sb_title_text}**")
    sb_sample = forecast[(forecast['ds'] >= last_sb_dt - datetime.timedelta(days=lower_win+5)) & (forecast['ds'] <= last_sb_dt + datetime.timedelta(days=upper_win+5))]
    sb_chart = alt.Chart(sb_sample).mark_area(color='#fbbf24', opacity=0.6).encode(
        x=alt.X('ds:T', title=sb_title_text),
        y=alt.Y('superbowl:Q', title="Score Adjustment"),
        tooltip=[alt.Tooltip('ds:T', title='Date'), alt.Tooltip('superbowl:Q', title='Effect', format='.2f')]
    )
    st.altair_chart(sb_chart.properties(width='container', height=250).interactive(), width='stretch')