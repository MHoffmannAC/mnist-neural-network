...
def draw_network_svg(layers_config, activations=None, model=None, saliencies=None):
    ...
    for idx in in_indices:
        if idx == "dots":
            in_nodes.append({'id': 'dots', 'val': 0, 'rf': '', 'sal': '', 'real_idx': None})
        else:
            in_nodes.append({'id': f'in_{idx}', 'val': (activations[0][idx] if activations else 0), 'rf': '', 'sal': '', 'real_idx': idx})
    rendered_layers.append(in_nodes)
    
    # Hidden layers
    for l_idx, units in enumerate(layers_config):
        layer_acts = activations[l_idx + 2] if activations and len(activations) > l_idx + 2 else np.zeros(units)
        nodes = []
        for i in range(units):
            rf_b64 = get_rf_base64(model, l_idx + 1, i) if model else ""
            sal_b64 = array_to_base64(saliencies[l_idx + 1][i]) if saliencies and len(saliencies) > l_idx + 1 else ""
            nodes.append({'id': f'h_{l_idx}_{i}', 'val': layer_acts[i], 'rf': rf_b64, 'sal': sal_b64, 'real_idx': i})
    ...
    for i in range(10):
        ...
        out_nodes.append({'id': f'out_{i}', 'val': out_acts[i], 'rf': rf_b64, 'sal': sal_b64, 'real_idx': i})
    ...
    for c_idx, layer in enumerate(rendered_layers):
        ...
        for r_idx, node in enumerate(layer):
            y = (height / 2) - ((len(layer) * min(row_height, 40)) / 2) + (r_idx * min(row_height, 40)) + 20
            layer_coords.append((x, y, node['val'], node['id'], node['rf'], node['sal'], row_height, node['real_idx']))
    ...
    for i in range(len(coords) - 1):
        ...
        if max_src < 1e-6: max_src = 1.0
        if max_dst < 1e-6: max_dst = 1.0

        for node_a in coords[i]:
            if node_a[3] == 'dots': continue
            norm_src = float(np.clip(node_a[2] / max_src, 0, 1))
            
            for node_b in coords[i+1]:
                if node_b[3] == 'dots': continue
                norm_dst = float(np.clip(node_b[2] / max_dst, 0, 1))
                min_opacity = 0.15
                opacity = min(0.9, min_opacity + norm_dst * 0.4 + (norm_src * norm_dst) * 0.35) if activations else 0.1
                min_width = 0.8
                stroke_w = min_width
                color = "#475569" 
                ...
                svg_lines.append(f'<line x1="{node_a[0]}" y1="{node_a[1]}" x2="{node_b[0]}" y2="{node_b[1]}" stroke="{color}" stroke-width="{stroke_w}" opacity="{opacity}" />')
    ...
    for c_idx, layer in enumerate(coords):
        ...
        for node in layer:
            ...
            if nid == 'dots':
                svg_nodes.append(f'<text x="{x}" y="{y}" fill="#64748b" font-size="20" text-anchor="middle">⋮</text>')
                continue
            
            intensity = float(np.clip(val, 0, 1)) if is_output else float(np.clip(val / layer_max, 0, 1))
            if not activations: intensity = 0.0
            ...
            svg_nodes.append(f'<circle cx="{x}" cy="{y}" r="{r}" fill="{bg_color}" stroke="#1e293b" stroke-width="2" />')
            
            if is_output:
                label = nid.split('_')[1]
                svg_nodes.append(f'<text x="{x+20}" y="{y+4}" fill="#e2e8f0" font-family="monospace" font-size="12">{label}</text>')
            ...
            if rf_b64:
                ...
                sal_svg = ""
                if saliencies:
                    img_data = sal_b64 if sal_b64 else "iVBORw0KGgoAAAANSUhEUgAAAFQAAABUCAYAAAAcaxDBAAAAAXNSR0IArs4c6QAAAExJREFUeF7twTEBAAAAwqD1T20ND6AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAOBvAHAQAAH8S88fAAAAAElFTkSuQmCC"
...
with st.sidebar:
    ...
    num_hidden = st.number_input("Hidden Layers", 0, 5, len(st.session_state.layers_config))
    new_layers_config = []
    for i in range(num_hidden):
        prev_val = st.session_state.layers_config[i] if i < len(st.session_state.layers_config) else 16
        units = st.slider(f"Neurons in Layer {i+1}", 1, 32, prev_val)
        new_layers_config.append(units)
        
    st.markdown("### Training Settings")
    train_size = st.selectbox("Training Data Size", [4000, 10000, 30000, 60000], index=3)
    ...
    update_freq = st.selectbox("Update UI every N Epochs", [1, 2, 5, 10, 20, 50, 100], index=0)
    
    current_config = {'layers': new_layers_config, 'activation': activation, 'lr': lr, 'train_size': train_size}
    ...
    with col1:
        if st.button("Train / Resume" if not st.session_state.is_training else "⏸ Pause"):
            if st.session_state.current_epoch < epochs:
                st.session_state.is_training = not st.session_state.is_training
                st.rerun()
    with col2:
        if st.button("Reset Model"):
            st.session_state.model = build_compiled_model(st.session_state.layers_config, activation, lr)
            st.session_state.current_epoch = 0
            st.session_state.history = {'loss': [], 'acc': [], 'val_acc': []}
            st.session_state.is_training = False
            st.rerun()
...
st.markdown(f"**Epoch:** `{st.session_state.current_epoch} / {epochs}` | Status: {'🟢 Training...' if st.session_state.is_training else '🟡 Paused / Idle'}")
...
with col_testing:
    ...
    input_method = st.radio("Input Method:", ["Draw Manually", "Random Example"], horizontal=True)
    ...
    if input_method == "Draw Manually":
        ...
        if canvas_result.image_data is not None:
            img_gray = np.mean(canvas_result.image_data[:, :, :3], axis=2).astype(np.uint8)
            if np.sum(img_gray) > 0:
                resized = np.clip(np.array(Image.fromarray(img_gray).resize((20, 20), Image.Resampling.LANCZOS)).astype('float32') / 255.0, 0, 1)
                canvas_28 = np.zeros((28, 28), dtype=np.float32)
                canvas_28[4:24, 4:24] = resized
                test_input = canvas_28.reshape(1, 28, 28, 1)
                
                with input_col:
                    st.markdown("<div style='font-size: 0.8rem; color: #94a3b8;'>Model Input (28x28):</div>", unsafe_allow_html=True)
                    st.image(canvas_28, width=100, clamp=True)
    else:
        if st.button("🎲 Load Random Image") or st.session_state.random_test_idx is None: 
    ...
    if test_input is not None and st.session_state.model:
        extractor = tf.keras.Model(inputs=st.session_state.model.inputs, outputs=[l.output for l in st.session_state.model.layers])
        ...
        st.markdown("<div style='margin-top: 10px; margin-bottom: 5px;'><b>Softmax Confidence</b></div>", unsafe_allow_html=True)
        preds = activations[-1]
        
        pred_df = pd.DataFrame({
            'Digit': [str(i) for i in range(10)],
            'Confidence': preds,
            'Color': ["#10b981" if i == np.argmax(preds) else "#64748b" for i in range(10)]
        })
        ...
        conf_chart = alt.Chart(pred_df).mark_bar().encode(
            ...
            y=alt.Y('Confidence:Q', scale=alt.Scale(domain=[0, y_max]), axis=alt.Axis(title=None, format='.0%')),
...
if len(st.session_state.history['loss']) > 0:
    ...
    with c1:
        st.markdown(f"<div class='metric-container' style='margin-bottom: 0;'><div>Loss</div><div class='metric-value' style='color:#60a5fa'>{st.session_state.history['loss'][-1]:.4f}</div></div>", unsafe_allow_html=True)
        loss_chart = alt.Chart(pd.DataFrame({'Epoch': epochs_arr, 'Loss': st.session_state.history['loss']})).mark_line(color="#60a5fa").encode(
            x='Epoch', 
    ...
    with c2:
        ...
        st.markdown(f"<div class='metric-container' style='margin-bottom: 0;'><div>Train Acc</div><div class='metric-value' style='color:#34d399'>{tacc[-1]:.1f}%</div></div>", unsafe_allow_html=True)
        tacc_chart = alt.Chart(pd.DataFrame({'Epoch': epochs_arr, 'Train Acc': tacc})).mark_line(color="#34d399").encode(
            x='Epoch', 
            y=alt.Y('Train Acc', scale=alt.Scale(zero=False))
        ).properties(height=150)
        st.altair_chart(tacc_chart, use_container_width=True)
        
    with c3:
        vacc = [val * 100 for val in st.session_state.history['val_acc']]
        st.markdown(f"<div class='metric-container' style='margin-bottom: 0;'><div>Val Acc</div><div class='metric-value' style='color:#a78bfa'>{vacc[-1]:.1f}%</div></div>", unsafe_allow_html=True)
        vacc_chart = alt.Chart(pd.DataFrame({'Epoch': epochs_arr, 'Val Acc': vacc})).mark_line(color="#a78bfa").encode(
            x='Epoch', 
            y=alt.Y('Val Acc', scale=alt.Scale(zero=False))
        ).properties(height=150)
        st.altair_chart(vacc_chart, use_container_width=True)
else: 
